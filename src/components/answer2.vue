<template>
  ...
  <div style="width:100%;height:100%;z-index:3;">
    <el-row class="borderDiv" style="padding:19px;height:9%">
      <el-col :span="1">
        <icon src="back.svg" hover="backh.svg" href="/video2" />
      </el-col>
      ...
      <el-col :span="6" style="height:100%">
        <el-row class="tab_left" style="height:35%">
          <el-col :span="22" :offset="1" style="height:100%">
            <VideoBg :sources="['/static/video/n2.mp4']" ></VideoBg>
          </el-col>
        </el-row>
...
import log from '@/components/log.vue'
  ...
  export default {
    ...
    data() {
      return {
        playerOptions: {
          ...
          sources: [{
            type: "video/mp4", // 类型
            src: '/static/video/n2.mp4' // url地址
          }],
          poster: '/static/video/n2-bg.PNG', // 封面地址
    ...
    mounted () {
      this.$api.get("getAnswer?video=v2").then(res=>{
        if (res.result == -1) {
            this.$router.push('/video2')
            return
        }
        for(let a of res.data) {
          a.picture = this.$api.globalPath + "/v2/" + a.username + "/picture.jpg"
          a.reasonPic = []
          for(let i=1;i<=a.reasons;i++){
            a.reasonPic.push(this.$api.globalPath + "/v2/" + a.username + `/reason${i}.jpg`);
          }
